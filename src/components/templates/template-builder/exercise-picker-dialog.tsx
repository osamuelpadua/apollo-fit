...
import { addTemplateExercise, type AddedTemplateExercise } from "@/features/templates/actions"
...
export function TemplateExercisePicker({ sectionId, onAdd }: Props) {
  ...
  async function handleAdd(exercise: Exercise) {
    ...
    try {
      ...
      const d: AddedTemplateExercise = result.data
      const row: TExRow = {
        id: d.id,
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={handleOpen}>
        <DialogContent
          className="bg-card border-border max-w-2xl flex flex-col gap-0 p-0 overflow-hidden"
        ...
        >
          ...
          <div className="px-5 pt-4 space-y-3">
            ...
            <div className="flex gap-1.5 overflow-x-auto pb-1">
              <button
                onClick={() => setMuscle("")}
                className={`shrink-0 rounded-full px-2.5 py-0.5 text-xs border transition-colors ${!muscle ? "bg-primary/10 text-primary border-primary/20" : "border-border text-muted-foreground hover:text-foreground"}`}
              >
                Todos
              </button>
              {muscleGroups.map(([value, label]) => (
                <button
                  ...
                  className={`shrink-0 rounded-full px-2.5 py-0.5 text-xs border transition-colors ${muscle === value ? "bg-primary/10 text-primary border-primary/20" : "border-border text-muted-foreground hover:text-foreground"}`}
                >
                  {label}
                </button>
              ))}
          ...
          <div
            className="flex-1 overflow-y-auto px-5 py-3 space-y-1 min-h-0"
